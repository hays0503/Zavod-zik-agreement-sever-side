...
const resolvers = {
    ...
    Query: {
        ...
        data_agreement_list_internal_needs: async (parent, args, context) => {
            let [dbQuery] = queryParseJson({ query: context.body.query, variables: args ? args : context.body.variables, tables });
            const res = await client.query(dbQuery.data_agreement_list_internal_needs);
            return res.rows;
        },
    ...
    Mutation: {
        ...
        updateDocumentTasks: async (parent, args) => {
            await client.query(`SELECT * FROM document_tasks_update('${JSON.stringify(args.document_tasks)}')`);
            publish('document_tasks', client);
            return { type: 'success', message: 'Успешно создано' };
        },
        ...
        deleteSignature: async (parent, args) => {
            await client.query(`SELECT * FROM document_signature_delete(('${JSON.stringify(args.signature)}')`);
            publish('document_signatures', client);
            return { type: 'success', message: 'Успешно изменено' };
        },
        // -----Files mutatuions-----
        deleteFile: async (parent, args) => {
            await client.query(`SELECT * FROM files_delete('${JSON.stringify(args.document_files)}')`);
            publish('documents', client);
        ...
        insertForm: async (parent, args) => {
            await client.query(`SELECT * FROM admin_form_settings_insert('${JSON.stringify(args.forms)}')`);
            publish('forms', client)
            return { type: 'success', message: 'Успешно' };
        },
        updateForm: async (parent, args) => {
            await client.query(`SELECT * FROM admin_form_settings_update('${JSON.stringify(args.forms)}')`);
            publish('forms', client)
            return { type: 'success', message: 'Успешно' };
        },
        deleteForm: async (parent, args) => {
            await client.query(`SELECT * FROM admin_form_settings_delete('${JSON.stringify(args.forms)}')`);
            publish('forms', client)
            return { type: 'success', message: 'Успешно' };
        },
        //Document routes
    ...
    Subscription: {
        ...
        document_tasks: {
            subscribe: (parent, args, context) => {
                let [dbQuery] = queryParseJson({ query: context.connection.query, variables: args, tables });
                return pubSub.asyncIterator([`document_tasks$query$${dbQuery.document_tasks}`])
            }
        },
        ...
        document_logs: {
            subscribe: (parent, args, context) => {
                let [dbQuery] = queryParseJson({ query: context.connection.query, variables: args, tables });
                return pubSub.asyncIterator([`document_logs$query$${dbQuery.document_logs}`])
            }
        },
        document_tasks_logs: {
            subscribe: (parent, args, context) => {
                let [dbQuery] = queryParseJson({ query: context.connection.query, variables: args, tables });
                return pubSub.asyncIterator([`document_tasks_logs$query$${dbQuery.document_tasks_logs}`])
            }
        }
