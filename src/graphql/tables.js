const tables = {
    ...
    "documents": {
        "fields": {
            ...
            "document_logs":"ARRAY(SELECT row_to_json(j.*) FROM (SELECT $*$ FROM document_logs AS $Q++$ WHERE document_id IN (SELECT id FROM documents WHERE id = $Q$.id) and is_read=false $ORDER_BY$) as j ) AS document_logs ",
            "data_agreement_list":"ARRAY(SELECT row_to_json(j.*) FROM (SELECT $*$ FROM data_agreement_list AS $Q++$ WHERE document_id IN (SELECT id FROM documents WHERE id = $Q$.id) $WHERE$ $ORDER_BY$) as j ) AS data_agreement_list ",
            "data_one":"ARRAY(SELECT row_to_json(j.*) FROM (SELECT $*$ FROM data_one AS $Q++$ WHERE document_id IN (SELECT id FROM documents WHERE id = $Q$.id) $WHERE$ $ORDER_BY$) as j ) AS data_one ",
            "data_agreement_list_production":"ARRAY(SELECT row_to_json(j.*) FROM (SELECT $*$ FROM data_agreement_list_production AS $Q++$ WHERE document_id IN (SELECT id FROM documents WHERE id = $Q$.id) $WHERE$ $ORDER_BY$) as j ) AS data_agreement_list_production ",
            "data_agreement_list_internal_needs":"ARRAY(SELECT row_to_json(j.*) FROM (SELECT $*$ FROM data_agreement_list_internal_needs AS $Q++$ WHERE document_id IN (SELECT id FROM documents WHERE id = $Q$.id) $WHERE$ $ORDER_BY$) as j ) AS data_agreement_list_internal_needs ",
            "data_custom":"ARRAY(SELECT row_to_json(j.*) FROM (SELECT $*$ FROM data_custom AS $Q++$ WHERE document_id IN (SELECT id FROM documents WHERE id = $Q$.id) $WHERE$ $ORDER_BY$) as j ) AS data_custom ",
            "document_tasks": "ARRAY(SELECT row_to_json(j.*) FROM (SELECT $*$ FROM document_tasks AS $Q++$ WHERE document_id IN (SELECT id FROM documents WHERE id = $Q$.id) $WHERE$ $ORDER_BY$) as j ) AS document_tasks",
            "user_id": "bigint",
            "username": "character varying",
        ...
        "where": {
            ...
            "is_read": "is_read $*$",
            "positions": "exists (select id from (select id, json_array_elements(route_data) as elem from documents) as docelem where(elem->> 'positionId'):: int = any(array[$*$]))",
            "approved_by_me": "id in(select document_id from document_signatures where user_id=$*$)",
            "documents_for_receiver":"id in(select document_id from document_tasks where user_id_receiver=$*$)",
    ...
    "document_tasks":{
        "fields":{
            ...
            "document_id":"bigint",
            "status":"bigint",
            "task_statuses": "(SELECT row_to_json(j.*) AS task_statuses FROM (SELECT $*$ FROM task_statuses AS $Q++$ WHERE id = $Q$.status) as j)",
            "is_cancelled":"boolean",
            "note":"character varying",
            "deadline":"character varying",
            "document_tasks_logs":"ARRAY(SELECT row_to_json(j.*) FROM (SELECT $*$ FROM document_tasks_logs AS $Q++$ WHERE task_id IN (SELECT id FROM document_tasks WHERE id = $Q$.id) and is_read=false $ORDER_BY$) as j ) AS document_tasks_logs ",
            "date_created":"timestamp without time zone",
            ...
            "task_files":"bigint",
            "document_tasks_files":"ARRAY(SELECT row_to_json(j.*) FROM (SELECT $*$ FROM document_tasks_files AS $Q++$ WHERE filename is not null and task_id IN (SELECT id FROM document_tasks WHERE id = $Q$.id) $WHERE$ $ORDER_BY$) as j ) AS document_tasks_files",
