...
let queryParseJson = ({ query, variables, tables }) => {
  ...
  let queryParse = ({ fields, variables, subscriptionName }) => {
    ...
    let parse = (queryJson, queryStringFrom, localWhere) => {
      ...
      for (let i = 0; i < queryJson[key].fields.length; i++) {
        if (i !== 0 && i !== queryJson[key].fields.length) {
          queryStringFields += ", ";
        }
        ...
        if (typeof queryJson[key].fields[i] == "string") {
          ...
          let query = tables[queryStringFrom]
            ? tables[queryStringFrom].fields[
                Object.keys(queryJson[key].fields[i])[0]
              ]
            : tables[
                Object.keys(tables).filter((tableKey) => {
                  return tables[tableKey].comment === queryStringFrom;
                })[0]
              ].fields[Object.keys(queryJson[key].fields[i])[0]];

          if (localWhere != null && tables[queryStringFrom]) {
            if (
              Object.keys(queryJson[key].fields[i])[0] ===
              Object.keys(localWhere)[0]
            ) {
              if (
                Object.keys(
                  localWhere[Object.keys(queryJson[key].fields[i])[0]]
                ).filter((item) => {
                ...
                delete localWhere[Object.keys(queryJson[key].fields[i])[0]][
                  "ORDER_BY"
                ];
              }
      ...
      if (variables[queryStringFrom] && subQuery === 0) {
        ...
        if (variables[queryStringFrom].global) {
          if (
            Object.keys(variables[queryStringFrom].global).filter((item) => {
              return item === "ORDER_BY";
            }).length !== 0
          ) {
            variables[queryStringFrom].global["ORDER_BY"].forEach(
              (value, index) => {
          ...
          if (Object.keys(variables[queryStringFrom].global).length !== 0) {
            WHERE = parseWhere(
              variables[queryStringFrom].global,
              queryStringFrom
            );
          }
        }
      }
      return `${subQuery === 0 ? "SELECT " : ""}${queryStringFields} ${
        subQuery === 0 ? `FROM ${queryStringFrom} AS Q0` : ""
      }${WHERE !== "" && subQuery === 0 ? ` WHERE ${WHERE}` : ""}${
        ORDER_BY !== "" && subQuery === 0 ? ` ORDER BY ${ORDER_BY}` : ""
      }`;
    };

    fields.map((value, index, array) => {
      let from = Object.keys(fields[index])[0].slice(
        0,
        Object.keys(fields[index])[0].length
      );
      responseJson[Object.keys(fields[index])[0]] = parse(
        value,
        from,
        variables[from] ? variables[from].local : null
      );
    ...
    if (Array.isArray(where)) {
      ...
      Object.keys(where).forEach((key, i, array) => {
        ...
        if (typeof where[key] == "string") {
          queryStringWhere += tables[queryStringFrom].where[key].replace(
            "$*$",
            `${where[key]}`
          );
        } else {
          ...
          let WHERE = tables[queryStringFrom]
            ...
            : tables[
              ...
              ].where[Object.keys(where[key])[0]];
          let FROM = tables[key]
            ? key
            : Object.keys(tables).filter((tableKey) => {
                return tables[tableKey].comment === key;
              })[0];

          queryStringWhere += WHERE.replace(
            "$WHERE$",
            parseWhere(where[key], FROM)
          );
        }
      });
    }
  ...
  let ttt = queryParse({
    fields,
    variables,
    subscriptionName: queryName.split(" ")[1],
  });
  //console.log(ttt);
  return ttt;
};
...
let now = (timestamp = new Date()) => {
  return `${new Date(timestamp - timestamp.getTimezoneOffset() * 60000)
    .toISOString()
    .slice(0, -5)
    .replace(/T/, " ")}`;
};
...
const skudLogger = (file, message, fs) => {
